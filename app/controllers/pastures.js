...
exports.createPastures = function(req, res, next) {
    var sql = mysql.format("INSERT INTO Pastures SET ?", req.body);
    config.query(req, res, next, sql, req.query.token);
};
exports.updatePastures = function(req, res, next) {
    var token = req.query.token;
    delete req.query.token;
    var sql = mysql.format("UPDATE Pastures SET ? WHERE pastureId = ?", [req.body, req.query.pastureId]);
    config.query(req, res, next, sql, token);
};
exports.deletePastures = function(req, res, next) {
    var sql = mysql.format("DELETE FROM Pastures WHERE pastureId = ?", [req.query.pastureId]);
    config.query(req, res, next, sql, req.query.token);
};
